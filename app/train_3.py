...
from model.model_combine import model_selectorBoth
...
def train(args):
    ...
    with open("11Janlan1_Train3_CV50_w2v_Glove2_cnn3xStatic.txt", 'wb') as result_CV:
        for list in range(0, 10):
            ...
            for i in range(0, fold):
                ...
                #           batch_size=args.batch_size, callbacks=callbacks_list)
                model.fit([x_train, x_train, x_train, x_train], y_train, epochs=args.num_epochs, batch_size=args.batch_size, verbose=0)
                # print("Test model ...")
                ...
                scores = model.evaluate([x_val,x_val, x_val,x_val], y_val, verbose=0)
            ...
            np.mean(cv_scores), np.std(cv_scores)) + " ROC: %.2f%% (+/- %.2f%%)" % (
