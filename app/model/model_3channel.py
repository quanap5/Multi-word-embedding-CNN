...
def model_selector3(args, embedding_matrix):
...
def _predefined_model(args, embedding_matrix):
    ...
    '''This part use merge from keras to combine 3 channels'''
    ...
    both_channel = Merge([text_channel1, text_channel2, text_channel3], mode='concat')


    input_node = Input(shape=(args.max_sequence_len, 3*args.embedding_dim))
    conv_list = []
    ...
    model.add(Dropout(dropout_list[0], input_shape=(args.max_sequence_len, 3*args.embedding_dim)))
    model.add(graph)
    model.add(Dense(150))
